...
was innocently proposed by Jenny Bryan on Twitter on April 24,
2017. It was an obscure, ha-ha-ha in-joke reference to an album cover for the record Unknown Pleasures by the British punk-rock band Joy Division. That cover used a set of [plots of radio frequency intensities of the first recorded pulsar](https://blogs.scientificamerican.com/sa-visual/pop-culture-pulsar-the-science-behind-joy-division-s-unknown-pleasures-album-cover/). Yet it turned out that the band name “Joy Division” is a reference to a 1955 novella, _House of Dolls_ which
described the groups of Jewish women kept as sexual slaves in concentration camp brothels called “Joy Divisions” (Freuden-abteilungen) during World War II. 
Claus Wilke called his implementation package `ggjoy`, but after being apprised of the
awful reference [changed it to ridgeline plot](https://clauswilke.com/blog/2017/09/15/goodbye-joyplots/) in the `ggridges` package.
...
# probly <- read_csv(here::here("blog", "drafts", "BeachScience", "probly.csv"))  # local cache, if offline
