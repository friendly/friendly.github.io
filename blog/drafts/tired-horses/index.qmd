...
description: "Writing technical, code-dependent material usually pulls you into necessary detours -- fixing a package, chasing a citation, syncing across machines, juggling AI sessions -- and each detour can spawn its own. A look at where the friction actually comes from."
...
When I write my books or articles, my subconscious often thinks about the **writing process**: what it takes to "Get Some Writing Done".
It's not simply about putting words down on a page, or typing them into an editor
or word processor window.

Over the years that's meant journal articles, book projects, and, now, this blog --
a happy family in some ways, the way siblings share a last name and a few
mannerisms. In writing, they call on the same core skills: organizing an argument, drafting,
revising, cutting what doesn't earn its place. 

But each is unhappy, or at least
...
paragraph or lose it. (John McPhee's [*Draft No. 4: On the Writing
Process*](https://us.macmillan.com/books/9780374537975/draftno4/) is the best
book-length treatment I know of this same restlessness about how writing actually
gets done.) Different shapes, same underlying process.

The core skills are probably fine for fiction or non-fiction, but,
...
I can't help but think about graphical methods I use for data analysis and
see how they actually work in the context I'm writing about. Most often, I can just 
recall how I did this before, or find a new package that does it better.
But often in this work I have to wear a Developer hat, because these methods are
...
A smaller but more consequential example is the idea of a general mechanism for flagging and
...
a plot is showing. You get a hint of this already in the diagnostic panels produced by
`plot(lm(y ~ x1 + x2 + ...))`, where unusual points get automatically labeled by case name --
but only if your data frame actually has `rownames()`.

This is one of the most endearing aspects of the `car` package. Its plotting functions --
...
None of that existed for `ggplot2`, though. Finding and labeling unusual points is another data step and `geom_*()`s
you have to wire up yourself, one plot at a time--- every single time!
heplots::noteworthy()`, and its `ggplot2`
layer `stat_noteworthy()` (still sitting in `dev/`), ports the same vocabulary -- `"mahal"`,
`"x"`, `"y"`, `"r"`, or a Cook's-distance-style numeric vector -- into the grammar of graphics,
so any `ggplot`, not just `car`'s own base-graphics functions, gets the same treatment. It's
...
<!-- TODO(Michael): fill in where this actually mattered for the book, or whether it's still
     purely a "would be nice" -- I only have the code and what it does, not the story of why
     it came up or whether it's shipping. -->

## 🚧 Detour 2 — SLOW: MERGING TRAFFIC: chasing the perfect citation and the perfect image

Historical papers -- [Guerry](https://www.datavis.ca/papers/guerry-STS241.pdf),
[van Langren](https://www.datavis.ca/papers/vita/Friendly-etal2010langren.html),
[Minard](https://www.datavis.ca/papers/Raiders_Lost_Tombs.pdf) -- run into a different kind
of detour. There's a general theme and structure going in, but the devil is in the details
of two recurring tasks.
