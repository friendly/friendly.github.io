...
what happens to a branch once it's grown. `ggbiplot` was a small branch (fix one figure for
one chapter) that I kept watering until it became a permanent limb of the tree -- ongoing
maintenance, not a detour anymore. `tinyplot` was a branch I looked at, admired, and cut, not
because it was a bad branch but because keeping it alive meant re-growing seventy other
branches around it. `heplots::noteworthy()`'s `ggplot2` layer is a branch still on the tree,
