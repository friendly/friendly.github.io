...
in LaTeX. Nothing was generated or renamed by an intermediate tool.
...
One nice feature was the ability to define **global variables** in LaTeX and in R. In Quarto,
every chapter listed in the `_quarto.yml` file is compiled separately, so anything common
must be replicated in each chapter.

<!--
TODO: The above should be a separate section, using as illustrations the `R/common.R` file run in each chapter, as well as the
LaTeX definitions inserted using `{{}}` in the chapters, keeping track of R packages loaded, and other tasks.
